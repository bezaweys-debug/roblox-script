...
-- Fungsi Filter (Mengabaikan "spawn", "base", dan nama yang mengandung "_egg")
local function isValidEgg(name)
	local lowerName = string.lower(name)
	
	-- Abaikan jika mengandung kata "spawn", "base", atau "_egg"
	if string.find(lowerName, "spawn") or string.find(lowerName, "base") or string.find(lowerName, "_egg") then
		return false
	end
...
-- 7. Fungsi Mengambil Warna Fisik dari Bagian Telur
...
local function createEggLabel(eggPart, originalName)
	if not eggPart or eggPart:FindFirstChild("GlobalEggBillboard") then return end
	
	local eggColor = getPartColor(eggPart)
		...
		for _, descendant in ipairs(Workspace:GetDescendants()) do
			if isValidEgg(descendant.Name) then
				if descendant:IsA("BasePart") or descendant:IsA("Model") then
					...
					if targetPart then createEggLabel(targetPart, descendant.Name) end
				end
			end
		end
	else
